...
// ============================================================
// 全カテゴリ一括インポート（定期実行用）
// ⚠️ 2026-08-03 サイト運営停止に伴い、月次トリガーは削除済み。
//    トリガーを再設定しないこと（認可切れで毎月1日に失敗通知メールが届く）。
//    手動で1回だけ実行したい場合は、エディタから monthlyImportAll を直接実行する。
// ============================================================
...
// ⚠️ 2026-08-03 実行禁止。サイト運営停止済みのため、この関数を実行すると
//    毎月1日 午前2時のトリガーが復活し、認可切れで失敗通知メールが再発する。
//    トリガーを消したい場合は script.google.com/home/triggers から削除する。
// ============================================================
